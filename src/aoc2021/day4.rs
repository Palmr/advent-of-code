use aoc2021::day4::MarkableCell::{Marked, Unmarked};
use std::fmt;
...
/// at random, and the chosen number is marked on all boards on which it appears.
...
impl fmt::Display for MarkableCell {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            MarkableCell::Unmarked(num) => {
                write!(f, " {:>2} ", num)
            }
            MarkableCell::Marked(num) => {
                write!(f, "[{:>2}]", num)
            }
        }
    }
...
struct BingoBoard {
    cells: Vec<MarkableCell>,
}

impl BingoBoard {
    pub fn mark_number(&mut self, number: usize) -> bool {
        match self.cells.iter().position(|c| c == &Unmarked(number)) {
            None => {}
        ...
        self.cells
            ...
            .enumerate()
            .for_each(|(idx, cell_marked)| {
                if cell_marked {
                    rows_marked[idx / BOARD_SIZE] += 1;
                    cols_marked[idx % BOARD_SIZE] += 1;
                }
            });

        rows_marked.iter().any(|&count| count == BOARD_SIZE)
            || cols_marked.iter().any(|&count| count == BOARD_SIZE)
    }
...
impl fmt::Display for BingoBoard {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        self.cells
            .iter()
            .enumerate()
            .map(|(index, cell)| ((index + 1) % BOARD_SIZE == 0, cell))
            .for_each(|(end_of_row, cell)| {
                write!(f, "{} {}", cell, if end_of_row { "\n" } else { "" }).unwrap()
            });
        Ok(())
    }
}
            ...
            cells: board_rows
                ...
                .flat_map(|row| {
                    row.split(' ')
                        ...
                        .map(|num| Unmarked(num.parse::<usize>().unwrap()))
                })
                .collect(),
        })
        .collect();
...
pub fn solve_part_one(input: &[String]) -> usize {
    ...
    panic!("Did not find the winning board")
}

pub fn solve_part_two(input: &[String]) -> usize {
    let (draw_numbers, mut boards) = parse_input(input);

    let mut completed_boards: Vec<(usize, usize)> = Vec::new();
    for num in draw_numbers {
        for (idx, board) in boards
            .iter_mut()
            .enumerate()
            .collect::<Vec<(usize, &mut BingoBoard)>>()
        {
            if !completed_boards
                .iter()
                .any(|(completed_idx, _)| completed_idx == &idx)
                && board.mark_number(num)
            {
                completed_boards.push((idx, board.score(num)));
            }
        }
        if completed_boards.len() == boards.len() {
            return completed_boards.last().unwrap().1;
        }
    }

    panic!("Did not find the last winning board")
}

#[cfg(test)]
mod tests {
    use aoc2021::day4::solve_part_one;
    use aoc2021::day4::solve_part_two;
    ...
    fn examples_part_one() {
        assert_eq!(
            4512,
            solve_part_one(&read_file_input("resources/2021/day4_example.txt"))
        );
    }
    ...
    fn examples_part_two() {
        assert_eq!(
            1924,
            solve_part_two(&read_file_input("resources/2021/day4_example.txt"))
        );
    }
}
